...
func TestClientUploadImage(t *testing.T) {
	...
	err := laptopStore.Save(laptop)
	require.NoError(t, err, "testify: cannot create laptop")
	...
	require.NoError(t, err, "testify cannot create stream")
	...
	err = stream.Send(req)
	require.NoError(t, err, "testify cannot send laptop info")
	...
	require.NoError(t, err, "tetsify cannot get response")
	require.NotZero(t, res.GetId(), "testify invalid laptopId")
	require.Equal(t, size, int(res.GetSize()))

	savedImagePath := fmt.Sprintf("%s/%s%s", testImageFolder, res.GetId(), imageType)
	require.FileExists(t, savedImagePath, "testify filenot exxist")
...
func startTestLaptopServer(t *testing.T, laptopStore *service.InMemoryLaptopStore, imageStore *service.DiskImageStore, ratingStore *service.InMemoryRatingStore) string {
